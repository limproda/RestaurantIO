...
export const getAllEmployees = async (req, res) => {
  try {
    ...
    res.json({ success: true, employees }); // Devolvemos el array de empleados como JSON
  } catch (err) {
    res.status(500).json({ success: false, message: "Error al cargar a los empleados" }); // Si hay un error, devolvemos como 500
  }
};
...
export const getEmployeeById = async (req, res) => {
  try {
    ...
    res.json({ success: true, user }); // Si se encuentra, se devuelve
  } catch (err) {
...
export const createEmployee = async (req, res) => {
  try {
    ...
    res.status(400).json({ success: false, message: "Error al crear al nuevo"}); // Mensaje de error si ha habido algún fallo
  }
};
...
export const updateEmployee = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).json({ success: false, message: "Error al actualizar empleado" }); // Mensaje de error
  }
};
...
export const deleteEmployee = async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(404).json({ success: false, message: "Empleado no encontrado" }); // Mensaje de error si no se encuentra 
    res.json({ success: true, message: "Empleado eliminado" }); // Mensaje de éxito
  } catch (err) {
    res.status(500).json({ success: false, message: "Error al eliminar empleado" }); // Mensaje que engloba el resto de errores
  }
};
