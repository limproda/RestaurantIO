...
export const userVerification = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return res.status(401).json({ status: false, message: "No se ha recibido token" }); // Error de token no recibido
    }

    jwt.verify(token, process.env.TOKEN_KEY, async (err, data) => {
      if (err) {
        return res.status(401).json({ status: false, message: "Token inválido" }); // Error de token inválido
      }
